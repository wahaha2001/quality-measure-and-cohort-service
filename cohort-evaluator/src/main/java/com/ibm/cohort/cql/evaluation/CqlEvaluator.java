...
public class CqlEvaluator {
    ...
    public CqlEvaluationResult evaluate( CqlEvaluationRequest request ) {
        return evaluate(request.getDescriptor(), request.getParameters(), Pair.of(request.getContextKey(), request.getContextValue()), request.getExpressions().stream().map(CqlTypedExpression::getExpression).collect(Collectors.toSet()), DEFAULT_CQL_DEBUG );
    }
    
    public CqlEvaluationResult evaluate( CqlEvaluationRequest request, CqlDebug debug ) {
        return evaluate(request.getDescriptor(), request.getParameters(), Pair.of(request.getContextKey(), request.getContextValue()), request.getExpressions().stream().map(CqlTypedExpression::getExpression).collect(Collectors.toSet()), debug );
    }
